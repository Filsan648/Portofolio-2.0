...
function ButtonStyle1({ text }: { text: string }) {
  return (
    <button className="group relative flex items-center justify-center overflow-hidden border border-black/20 px-10 py-4 transition-colors duration-500 hover:border-black">
      ...
      <span className="absolute inset-0 h-full w-0  transition-all duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:w-full"></span>

      {/* Flèche icône qui apparaît (optionnel mais très Awwwards) */}
      <span className="relative z-10 mr-0 flex items-center transition-all duration-500 group-hover:mr-4 ">
        <span className="font-medium tracking-tight">{text}</span>
      </span>

      {/* L'icône flèche cachée qui glisse vers l'intérieur */}
      <svg 
        className="relative z-10 h-4 w-4 -translate-x-4 opacity-0 transition-all duration-500 group-hover:translate-x-0 group-hover:opacity-100 " 
        fill="none" 
        viewBox="0 0 24 24" 
        stroke="currentColor"
      >
...
function ButtonStyle2({ text }: { text: string }) {
  return (
    <button className="group relative flex items-center bg-black text-white justify-center overflow-hidden border border-black/20 px-10 py-4 transition-colors duration-500 ">
      {/* Remplissage de gauche à droite */}
